...
void Scene::initScene(vkme::VulkanData* vulkanData, vkme::core::DescriptorSetAllocator * dsAllocator)
{
...
void RenderToTexture::initScenes()
{
    ...
    _scene1.initScene(_vulkanData, _descriptorSetAllocator.get());
}

void RenderToTexture::initMesh(Scene& scene)
{
    const std::string imagePath = vkme::PlatformTools::assetPath() + "country_field_sun.jpg";
    scene.textureImage = std::unique_ptr<vkme::core::Image>(
...
void RenderToTexture::drawBackground(VkCommandBuffer cmd, uint32_t currentFrame, const vkme::core::Image* depthImage)
{
...
void RenderToTexture::drawGeometry(
    ...
    vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, _scene1.pipeline );
    ...
    for (auto m : _scene1.models)
    {
        vkme::core::DescriptorSet* ds[] = {
            _scene1.sceneDataDescriptorSet.get()
        };
        m->draw(cmd, _scene1.pipelineLayout, ds, 1);
    }
